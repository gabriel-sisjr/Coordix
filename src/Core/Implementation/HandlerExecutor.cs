...
namespace Coordix.Implementation
{
    ...
    public class HandlerExecutor : IHandlerExecutor
    {
            ...
            where TNotification : INotification
        {
            ...
            object[] handlers = _provider.GetServices(handlerType).ToArray();
            System.Collections.Generic.IEnumerable<Task> tasksHandlers = handlers.Where(h => h != null).Select(handler => InvokeNotificationHandler(handlerType, handler!, notification, cancellationToken));
        ...
        {
            MethodInfo methodInfo = GetHandleMethodInfo(handlerType);
            ParameterExpression handlerParam = Expression.Parameter(typeof(object), "handler");
            ParameterExpression requestParam = Expression.Parameter(typeof(IRequest), "request");
            ...
            Type requestType = handlerType.GetGenericArguments()[0];

            UnaryExpression castHandler = Expression.Convert(handlerParam, handlerType);
            UnaryExpression castRequest = Expression.Convert(requestParam, requestType);
            MethodCallExpression call = Expression.Call(castHandler, methodInfo, castRequest, cancellationTokenParam);
            Expression<Func<object, IRequest, CancellationToken, Task>> lambda = Expression.Lambda<Func<object, IRequest, CancellationToken, Task>>(call, handlerParam, requestParam, cancellationTokenParam);
        ...
        private static Func<object, IRequest<TResponse>, CancellationToken, Task<TResponse>> CreateRequestHandlerWithResponseDelegate<TResponse>(Type handlerType)
        {
            ...
            UnaryExpression castRequest = Expression.Convert(requestParam, requestType);
            MethodCallExpression call = Expression.Call(castHandler, methodInfo, castRequest, cancellationTokenParam);
            Expression<Func<object, IRequest<TResponse>, CancellationToken, Task<TResponse>>> lambda = Expression.Lambda<Func<object, IRequest<TResponse>, CancellationToken, Task<TResponse>>>(call, handlerParam, requestParam, cancellationTokenParam);
        ...
        private static Func<object, INotification, CancellationToken, Task> CreateNotificationHandlerDelegate(Type handlerType)
        {
            MethodInfo methodInfo = GetHandleMethodInfo(handlerType);
            ParameterExpression handlerParam = Expression.Parameter(typeof(object), "handler");
            ParameterExpression notificationParam = Expression.Parameter(typeof(INotification), "notification");
            ParameterExpression cancellationTokenParam = Expression.Parameter(typeof(CancellationToken), "cancellationToken");

            // Extract the concrete notification type from INotificationHandler<TNotification>
            Type notificationType = handlerType.GetGenericArguments()[0];

            UnaryExpression castHandler = Expression.Convert(handlerParam, handlerType);
            UnaryExpression castNotification = Expression.Convert(notificationParam, notificationType);
            MethodCallExpression call = Expression.Call(castHandler, methodInfo, castNotification, cancellationTokenParam);
            Expression<Func<object, INotification, CancellationToken, Task>> lambda = Expression.Lambda<Func<object, INotification, CancellationToken, Task>>(call, handlerParam, notificationParam, cancellationTokenParam);
        ...
        private static Task InvokeNotificationHandler<TNotification>(Type handlerType, object handler, TNotification notification, CancellationToken cancellationToken)
            where TNotification : INotification
        {
            Func<object, INotification, CancellationToken, Task> delegateFunc = GetNotificationHandlerDelegate(handlerType);
        ...
        {
            Func<object, IRequest, CancellationToken, Task> delegateFunc = GetRequestHandlerDelegate(handlerType);
        ...
        {
            Func<object, IRequest<TResponse>, CancellationToken, Task<TResponse>> delegateFunc = GetRequestHandlerWithResponseDelegate<TResponse>(handlerType);
            return delegateFunc(handler, request, cancellationToken);
        }
    }
}
